...
export default function InquiryModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity duration-300">
      <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col rounded-3xl bg-card border border-border shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                {inquiryCart.length === 0 ? (
                  ...
                  <div className="divide-y divide-border/60 max-h-60 overflow-y-auto pr-1">
                    {inquiryCart.map((item) => (
                      ...
                      >
                        {/* Mini image */}
                        <div className="w-14 h-14 rounded-none overflow-hidden bg-accent/20 flex-shrink-0">
                          {typeof item.image === 'string' && item.image.trim() !== '' ? (
                            <img src={item.image.trim()} alt={item.name} className="w-full h-full object-cover rounded-none" />
                          ) : (
