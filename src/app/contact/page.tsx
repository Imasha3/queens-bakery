      ...
      <main className="flex-grow py-12">
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          <div className="bg-card border border-border p-6 rounded-2xl shadow-sm space-y-4">
            <h3 className="font-serif text-lg font-bold text-foreground">Inquiries & Orders</h3>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Place your inquiries directly using our website bag. Simply browse products, add them to your inquiry basket, and submit your request.
            </p>
          </div>
          
          <div className="bg-card border border-border p-6 rounded-2xl shadow-sm space-y-4">
            <h3 className="font-serif text-lg font-bold text-foreground">Operating Hours</h3>
            <p className="text-xs text-muted-foreground leading-relaxed">
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 mb-16">
          <div className="bg-card border border-border p-6 sm:p-8 rounded-3xl shadow-sm space-y-6">
            ...
            {submittedSuccess && (
              <div className="rounded-2xl bg-emerald-500/10 border border-emerald-500/25 p-4 text-center text-xs text-emerald-600 font-semibold animate-in fade-in slide-in-from-top-1 duration-200">
                Message sent successfully ✓ Thank you for reaching out to Queen's Bakery!
              </div>
            )}

            {errorMessage && (
              <div className="rounded-2xl bg-rose-500/10 border border-rose-500/25 p-4 text-center text-xs text-rose-600 font-semibold animate-in fade-in slide-in-from-top-1 duration-200">
                ⚠️ {errorMessage}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col gap-1.5 sm:col-span-2">
                  ...
                  <textarea
                    ...
                    className="rounded-xl border border-border bg-background px-4 py-3 text-xs text-foreground focus:outline-none focus:border-primary/60 transition-colors resize-none"
                  />
                </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full flex items-center justify-center rounded-xl bg-primary text-primary-foreground py-4 text-xs font-semibold shadow-md hover:opacity-95 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
              >
                {submitting ? (
